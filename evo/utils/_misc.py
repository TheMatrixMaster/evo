...
def rand_log_logistic(
    ...
    u = (
        stratified_with_settings(shape, device=device, dtype=torch.float64) * (max_cdf - min_cdf)
        + min_cdf
    )
...
def rand_log_uniform(shape, min_value, max_value, device="cpu", dtype=torch.float32):
    ...
    return (
        stratified_with_settings(shape, device=device, dtype=dtype) * (max_value - min_value)
        + min_value
    ).exp()
...
def freq_weight_nd(shape, scales=0, dtype=None, device=None):
    indexers = [
        [slice(None) if i == j else None for j in range(len(shape))] for i in range(len(shape))
    ]
